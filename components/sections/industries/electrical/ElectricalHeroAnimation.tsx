...
const ELECTRICAL_SCENARIOS = [
  {
    id: 0,
    title: "01. 11:15 PM Arcing Breaker Call",
    ...
    logs: [
      ...
      "Detected high-hazard keywords: 'sparkling breaker', 'burning smell'",
      "Triaged emergency status & verified local service zip code",
    ],
  },
  {
    id: 1,
    title: "02. Panel Safety Guidance",
    ...
    logs: [
      'Rex: "For your safety, please flip the main breaker shut-off switch immediately!"',
      "Caller confirmed main panel shut-off is OPEN and safe",
      "Rex paged on-call master electrician Dan for emergency dispatch",
    ],
  },
  {
    id: 2,
    ...
    logs: [
      "Job locked into ServiceTitan / Housecall Pro calendar for 7:30 AM",
      "Dispatched confirmation text with Tech arrival tracking link",
      "Pings on-call electrician phone & sent emergency dispatch payload",
    ],
  },
  {
    id: 3,
    ...
    logs: [
      "Electrician Dan installed heavy-duty 200A main panel & surge protector",
      'Gia sent 1-click Google Review SMS: "Thanks Marcus!"',
...
export default function ElectricalHeroAnimation() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setActiveStage((prev) => (prev + 1) % ELECTRICAL_SCENARIOS.length);
    }, 4500);
    return () => clearInterval(interval);
  }, [isAutoPlaying]);

  const scenario = ELECTRICAL_SCENARIOS[activeStage];
  ...
  return (
    <div className="relative w-full max-w-lg mx-auto rounded-[28px] border-4 border-ink/10 bg-ink p-5 sm:p-6 shadow-2xl text-white overflow-hidden">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h3 className="font-heading font-extrabold text-white text-base">Rex — Electrical Dispatcher</h3>
      ...
      <div className="relative z-10 grid grid-cols-4 gap-1.5 mb-4">
        {ELECTRICAL_SCENARIOS.map((s, idx) => (
          <button
            key={s.id}
            type="button"
      ...
      <div className="relative z-10 rounded-2xl border border-white/10 bg-white/5 p-4 backdrop-blur-md space-y-3.5">
        ...
        <div className="flex items-center justify-between border-b border-white/10 pb-2.5">
          <div className="flex items-center gap-2">
            {activeStage === 0 ? (
              <Zap className="size-4 text-coral-text" />
            ) : activeStage === 3 ? (
          ...
          >
            ...
            <div className="space-y-2">
              <p className="font-mono text-[10px] uppercase tracking-wide text-white/40 flex items-center gap-1.5">
                ...
                Rex Electrical Dispatch Log Execution
              </p>
      ...
      <div className="relative z-10 mt-4 flex items-center justify-between rounded-xl border border-white/10 bg-black/40 px-3.5 py-2.5 text-xs text-white/70">
        <span className="font-mono flex items-center gap-2 text-[11px]">
          ...
          <span>0 Missed Midnight Power Calls</span>
        </span>
        <span className="font-mono text-[11px] font-bold text-teal flex items-center gap-1">
          <span>ServiceTitan / Housecall Synced</span>
