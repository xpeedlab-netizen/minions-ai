...
import { CheckCircle2, Sparkles, ShieldCheck, Play, RotateCcw, ArrowRight, Flame, Snowflake } from "lucide-react";

const HVAC_SCENARIOS = [
  {
    id: 0,
    title: "01. 11:45 PM Emergency AC Call",
    ...
    logs: [
      ...
      "Triaged issue: Compressor failing during 98° heatwave",
      "Quoted $129 emergency diagnostic fee & verified service area",
    ],
  },
  {
    id: 1,
    title: "02. Live Diagnostic Triage",
    shortName: "AI Triage",
    badge: "Fee Verified",
    ...
    ticketEst: "$8,500 System Replacement Potential",
    logs: [
      "Caller confirmed $129 diagnostic fee",
      "Rex verified slot availability on ServiceTitan calendar",
      "Assigned Priority 1 Emergency Tag to lead profile",
    ],
  },
  {
    id: 2,
    ...
    logs: [
      "Job locked into ServiceTitan Calendar for 8:00 AM",
      "Dispatched confirmation text with Tech arrival window",
      "Pings on-call HVAC technician phone automatically",
    ],
  },
  {
    id: 3,
    ...
    logs: [
      "Tech completed new 16 SEER AC install",
      'Gia sent 1-click Google Review link: "Thanks Robert!"',
...
export default function HvacHeroAnimation() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setActiveStage((prev) => (prev + 1) % HVAC_SCENARIOS.length);
    }, 4500);
    return () => clearInterval(interval);
  }, [isAutoPlaying]);

  const scenario = HVAC_SCENARIOS[activeStage];
    ...
    <div className="relative w-full max-w-lg mx-auto rounded-[28px] border-4 border-ink/10 bg-ink p-5 sm:p-6 shadow-2xl text-white overflow-hidden">
        ...
        <div className="flex items-center justify-between border-b border-white/10 pb-2.5">
          <div className="flex items-center gap-2">
            {activeStage === 0 ? (
              ...
              <Snowflake className="size-4 text-teal" />
            )}
      ...
      <div className="relative z-10 mt-4 flex items-center justify-between rounded-xl border border-white/10 bg-black/40 px-3.5 py-2.5 text-xs text-white/70">
        <span className="font-mono flex items-center gap-2 text-[11px]">
          ...
          <span>0 Missed Peak Heatwave Calls</span>
        </span>
